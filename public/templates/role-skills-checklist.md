# Role-by-Role Skills Checklist
### A free worksheet from AdaptiveResume

> Check off only what's genuinely true for you. The goal is to FIND skills you forgot — never to pad.

## Walk through a normal shift (hour by hour)
List every distinct thing you handle in a typical day:
- ____________________
- ____________________
- ____________________

## Standards you work under (name only what applies)
- [ ] SPC / GD&T / Gauge R&R / MSA
- [ ] IATF 16949 / ISO 9001 / AS9100
- [ ] OSHA 10 / OSHA 30 / Lockout-Tagout / arc-flash
- [ ] NEC (electrical)
- [ ] CLIA / HIPAA awareness / BLS (healthcare-adjacent)
- [ ] EPA 608 / hazmat
- [ ] Other: ____________________

> For each: did you WORK to the standard, or do you HOLD the certification? Mark which — they're different.

## Tools, equipment & software you actually use
- Machines / equipment: ____________________
- Control systems: ____________________
- Software / ERP / scheduling: ____________________

## People & process you coordinate
- Who you work across: ____________________
- Paperwork / inspections / reporting you own: ____________________

## Confirmation check
- [ ] Every item is something I could demonstrate on day one
- [ ] "Worked to" vs "certified in" is marked correctly
- [ ] Nothing here is padding

---
Want this done with you? Our base build proposes role-relevant skills and asks you to confirm each one. $49 one-time · AdaptiveResume.example
